...
const topProducts = [
  { name: "Blue Dream", sales: 45, revenue: 2025.0, category: "Flower" },
  { name: "Gummy Bears", sales: 38, revenue: 950.0, category: "Edibles" },
...
const categoryData = [
  ...
  { name: "Edibles", sales: 89, revenue: 2670.0, percentage: 15.9 },
  { name: "Vapes", sales: 67, revenue: 3685.0, percentage: 21.9 },
  { name: "Concentrates", sales: 34, revenue: 3230.0, percentage: 19.2 },
  { name: "Pre-Rolls", sales: 45, revenue: 1575.0, percentage: 9.4 },
];
...
const employeeMetrics = [
  ...
  { name: "Mike Chen", sales: 10875.5, transactions: 76, avgOrder: 143.1 },
  {
    name: "Emma Rodriguez",
    sales: 9320.75,
    transactions: 68,
...
export default function Analytics() {
  ...
  const storeLocations = [
    ...
    {
      id: "main",
      name: "Cannabest Main Store",
      address: "123 Cannabis St, Portland, OR",
    },
    {
      id: "downtown",
      name: "Cannabest Downtown",
      address: "456 Main St, Portland, OR",
    },
    {
      id: "eastside",
      name: "Cannabest Eastside",
      address: "789 Division St, Portland, OR",
    },
  ];
  ...
  const currentLocationName =
    storeLocations.find((loc) => loc.id === selectedLocation)?.name ||
    "All Locations";
  ...
  const downloadEndOfDayPDF = () => {
    const monthlyPace =
      (endOfDayData.monthlySalesTotal / endOfDayData.dayOfMonth) *
      endOfDayData.daysInMonth;
  ...
  const downloadEndOfDayExcel = () => {
    ...
    const csvData = [
      ...
      [
        ...
        (endOfDayData.totalSales / endOfDayData.customerCount).toFixed(2),
      ],
      ...
      [
        "Day of Month",
        `${endOfDayData.dayOfMonth} of ${endOfDayData.daysInMonth}`,
      ],
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <Card>
                ...
                <CardContent>
                  ...
                  {timeframe === "today" && currentData.change && (
                    <div
                      className={`text-xs flex items-center ${currentData.change.revenue > 0 ? "text-green-600" : "text-red-600"}`}
                    >
                      {currentData.change.revenue > 0 ? (
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  {timeframe === "today" && currentData.change && (
                    <div
                      className={`text-xs flex items-center ${currentData.change.transactions > 0 ? "text-green-600" : "text-red-600"}`}
                    >
                      {currentData.change.transactions > 0 ? (
                        <TrendingUp className="w-3 h-3 mr-1" />
                      ) : (
                        <TrendingDown className="w-3 h-3 mr-1" />
                      )}
                      {Math.abs(currentData.change.transactions)}% from
                      yesterday
                    </div>
                  )}
                </CardContent>
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  {timeframe === "today" && currentData.change && (
                    <div
                      className={`text-xs flex items-center ${currentData.change.customers > 0 ? "text-green-600" : "text-red-600"}`}
                    >
                      {currentData.change.customers > 0 ? (
                        <TrendingUp className="w-3 h-3 mr-1" />
                      ) : (
                        <TrendingDown className="w-3 h-3 mr-1" />
                      )}
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  {timeframe === "today" && currentData.change && (
                    <div
                      className={`text-xs flex items-center ${currentData.change.avgOrderValue > 0 ? "text-green-600" : "text-red-600"}`}
                    >
                      {currentData.change.avgOrderValue > 0 ? (
                        <TrendingUp className="w-3 h-3 mr-1" />
                      ) : (
                        <TrendingDown className="w-3 h-3 mr-1" />
                      )}
                      {Math.abs(currentData.change.avgOrderValue)}% from
                      yesterday
                    </div>
                  )}
                </CardContent>
                    ...
                    .map((location) => {
                      const locationData =
                        locationBreakdownData[
                          location.id as keyof typeof locationBreakdownData
                        ];
                      const currentLocationData =
                        timeframe === "today"
                          ? locationData.today
                ...
                <CardHeader>
                  <CardTitle className="flex items-center justify-between">
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => setShowSalesTrendDialog(true)}
                    >
                      ...
                      <div className="absolute inset-0 flex items-end justify-between gap-1 px-2">
                        {salesTrendData.slice(-15).map((day, index) => {
                          const maxSales = Math.max(
                            ...salesTrendData.slice(-15).map((d) => d.sales),
                          );
                    ...
                    <div className="grid grid-cols-3 gap-4 pt-4 border-t">
                      <div className="text-center">
                        <div className="text-2xl font-bold text-green-600">
                          ...
                          {(
                            salesTrendData
                              .slice(-7)
                              .reduce((sum, day) => sum + day.sales, 0) / 7
                          ).toFixed(0)}
                        </div>
                      ...
                      <div className="text-center">
                        <div className="text-2xl font-bold text-blue-600">
                          $
                          {Math.max(
                            ...salesTrendData.slice(-30).map((d) => d.sales),
                          ).toFixed(0)}
                        </div>
                        <div className="text-sm text-muted-foreground">
                          Peak Day
          ...
          <TabsContent value="customers" className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
              ...
              <Card>
                <CardContent className="p-4 text-center">
                  <div className="text-2xl font-bold text-purple-600">
                    {customerInsights.loyaltyMembers}
                  </div>
                  <div className="text-sm text-muted-foreground">
                    Loyalty Members
                  </div>
              ...
              <Card>
                <CardContent className="p-4 text-center">
                  <div className="text-2xl font-bold text-orange-600">
                    {customerInsights.medicalPatients}
                  </div>
                  <div className="text-sm text-muted-foreground">
                    Medical Patients
                  </div>
          ...
          <TabsContent value="employees" className="space-y-6">
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  {(() => {
                    const totalStoreSales = employeeMetrics.reduce(
                      (sum, emp) => sum + emp.sales,
                      0,
                    );
                    const totalStoreTransactions = employeeMetrics.reduce(
                    ...
                    return employeeMetrics.map((employee, index) => {
                      const salesPercentage =
                        (employee.sales / totalStoreSales) * 100;
                      const transactionPercentage =
                        (employee.transactions / totalStoreTransactions) * 100;
                          ...
                          <div className="flex items-center justify-between mb-3">
                            ...
                            <div className="text-right">
                              ...
                              <div className="text-sm text-muted-foreground">
                                ...
                                {employee.avgOrder.toFixed(2)} avg
                              </div>
                            </div>
                          </div>
                    ...
                    <div className="grid grid-cols-3 gap-4 text-sm">
                      <div>
                        ...
                        <div className="font-semibold">
                          $
                          {employeeMetrics
                            .reduce((sum, emp) => sum + emp.sales, 0)
                            .toLocaleString()}
                        </div>
                      </div>
                      <div>
                      ...
                      onClick={() => {
                        const csvData = [
                          ...
                          [
                            "Average ASPD",
                            (
                              aspdData.reduce((sum, p) => sum + p.aspd, 0) /
                              aspdData.length
                            ).toFixed(1),
                          ],
                          [
                            "High Performers (>5/day)",
                        ...
                        link.setAttribute(
                          "download",
                          `ASPD_Report_${new Date().toISOString().split("T")[0]}.csv`,
                        );
                        link.style.visibility = "hidden";
                        document.body.appendChild(link);
                        link.click();
                                ...
                                <div class="header">
                                  ...
                                  <p><strong>Period:</strong> ${
                                    timeframe === "custom" && customDateRange
                                      ? `${new Date(customDateRange.startDate).toLocaleDateString()} - ${new Date(customDateRange.endDate).toLocaleDateString()}`
                                      : "current month"
                                  } (${aspdData[0]?.daysInRange || 15} days)</p>
                                  <p><strong>Generated:</strong> ${new Date().toLocaleString()}</p>
                                </div>
                  ...
                  {aspdData
                    ...
                    .map((product, index) => {
                      ...
                      return (
                        <div
                          key={product.id}
                          className={`grid grid-cols-6 gap-4 p-3 rounded-lg border ${isTopPerformer ? "bg-green-50 border-green-200" : "bg-gray-50"}`}
                        >
                          <div className="flex items-center gap-2">
          ...
          <TabsContent value="eod" className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    ${endOfDayData.totalTax.toFixed(2)}
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    ${endOfDayData.cashSales.toFixed(2)}
                  </div>
                  <div className="text-xs text-muted-foreground">
                    {(
                      (endOfDayData.cashSales / endOfDayData.totalSales) *
                      100
                    ).toFixed(1)}
                    % of total
                  </div>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    ${endOfDayData.debitSales.toFixed(2)}
                  </div>
                  <div className="text-xs text-muted-foreground">
                    {(
                      (endOfDayData.debitSales / endOfDayData.totalSales) *
                      100
                    ).toFixed(1)}
                    % of total
                  </div>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    ${endOfDayData.creditSales.toFixed(2)}
                  </div>
                  <div className="text-xs text-muted-foreground">
                    {(
                      (endOfDayData.creditSales / endOfDayData.totalSales) *
                      100
                    ).toFixed(1)}
                    % of total
                  </div>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    {endOfDayData.customerCount}
                  </div>
                  <div className="text-xs text-muted-foreground">
                    $
                    {(
                      endOfDayData.totalSales / endOfDayData.customerCount
                    ).toFixed(2)}{" "}
                    avg per customer
                  </div>
                </CardContent>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    ${endOfDayData.monthlySalesTotal.toLocaleString()}
                  </div>
                  <div className="text-xs text-muted-foreground">
                    Total sales for{" "}
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    $
                    {(
                      (endOfDayData.monthlySalesTotal /
                        endOfDayData.dayOfMonth) *
                      endOfDayData.daysInMonth
                    ).toLocaleString()}
                  </div>
                  <div className="text-xs text-muted-foreground">
                    Projected monthly total at current pace (day{" "}
                    {endOfDayData.dayOfMonth} of {endOfDayData.daysInMonth})
                  </div>
                </CardContent>
              </Card>
                      ...
                      <div className="flex items-center justify-between mb-3">
                        ...
                        >
                          {drawer.status === "exact"
                            ? "Exact"
                            : drawer.status === "over"
                              ? `$${drawer.variance.toFixed(2)} Over`
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    <div>
                      ...
                      <div className="font-semibold">
                        $
                        {endOfDayData.drawerBreakdowns
                          .reduce((sum, d) => sum + d.expectedClosing, 0)
                          .toFixed(2)}
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="font-semibold">
                        $
                        {endOfDayData.drawerBreakdowns
                          .reduce((sum, d) => sum + d.actualCounted, 0)
                          .toFixed(2)}
                      </div>
                    </div>
                    <div>
                      ...
                      <div
                        className={`font-semibold ${
                          endOfDayData.drawerBreakdowns.reduce(
                    ...
                    <div>
                      ...
                      <div className="text-sm">
                        {
                          endOfDayData.drawerBreakdowns.filter(
                            (d) => d.status === "exact",
                          ).length
                        }{" "}
                        Exact,{" "}
        ...
        {showSalesTrendDialog && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-semibold">
                  Sales Trend Details (Last 30 Days)
                </h2>
                <Button
                  variant="ghost"
                  onClick={() => setShowSalesTrendDialog(false)}
                >
                  <X className="w-4 h-4" />
                </Button>
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  ...
                  <div className="text-center p-4 bg-purple-50 rounded-lg">
                    <div className="text-2xl font-bold text-purple-600">
                      $
                      {Math.max(...salesTrendData.map((d) => d.sales)).toFixed(
                        2,
                      )}
                    </div>
