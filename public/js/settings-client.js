(function(){
  ...
  function currentStoreId(){
    try { const raw = localStorage.getItem('pos_store'); if (!raw) return 'default'; const s = JSON.parse(raw); return (s && s.id) ? String(s.id) : 'default'; } catch(_) { return 'default'; }
  }
  function readCookie(name){ try{ const m = document.cookie.match(new RegExp('(?:^|; )'+name.replace(/([.$?*|{}()\[\]\\\/\+^])/g,'\\$1')+'=([^;]*)')); return m ? decodeURIComponent(m[1]) : null; }catch(_){ return null; } }
  function writeCookie(name, value){ try{ document.cookie = `${name}=${encodeURIComponent(value)}; path=/; max-age=${COOKIE_MAX_AGE}`; }catch(_){} }

  async function httpGet(path, params){
    // Prefer posAuth if present; it automatically adds headers
    if (window.posAuth) {
      const res = await window.posAuth.apiRequest('get', path.replace(/^\/api/, ''), params||{});
  ...
  async function httpPost(path, body, params){
    if (window.posAuth) {
      const res = await window.posAuth.apiRequest('post', path.replace(/^\/api/, ''), params ? { ...(body||{}), ...(params||{}) } : (body||{}));
      if (res && res.success && res.data) return res.data;
      if (res && res.data) return res.data;
    }
  ...
  async function getFromServer(sid){
    // Try Laravel first
    try { return await httpGet('/api/settings/pos', { store: sid }); } catch(_) {}
  ...
  const SettingsClient = {
    ...
    loadLocal(sid){
      try { const s = localStorage.getItem(LS_KEY(sid)); if (s) return JSON.parse(s); } catch(_) {}
      try { const c = readCookie(CK_KEY(sid)); if (c) return JSON.parse(c); } catch(_) {}
      return null;
    },
    saveLocal(sid, settings){
      try { localStorage.setItem(LS_KEY(sid), JSON.stringify(settings)); } catch(_) {}
      try { writeCookie(CK_KEY(sid), JSON.stringify(settings)); } catch(_) {}
    },
    ...
    async get(force=false){
      ...
      if (!force){
        ...
        if (local) return { success: true, settings: { ...DEFAULTS, ...local } };
      }
      // Server with retries
      let last = null;
      for (let i=0;i<3;i++){
        try {
          ...
          const settings = (data && typeof data === 'object' && (data.settings || data)) ? (data.settings || data) : {};
      ...
      return { success: false, settings: { ...DEFAULTS, ...fallback }, error: last };
    },
    ...
    async save(patch){
      ...
      for (let i=0;i<3;i++){
        try {
          const data = await httpPost('/api/settings/pos', merged, { store: sid });
